...
sega_pipe = SemanticStableDiffusionPipeline(
    vae=sd_pipe.vae, text_encoder=sd_pipe.text_encoder, tokenizer=sd_pipe.tokenizer,
    unet=sd_pipe.unet, scheduler=sd_pipe.scheduler, safety_checker=None,
...
def invert(x0: torch.FloatTensor, prompt_src: str = "", num_inference_steps=100, cfg_scale_src=3.5, eta=1):
    ...
    with autocast("cuda"), inference_mode():
        w0 = (sd_pipe.vae.encode(x0).latent_dist.mode() * 0.18215).to(dtype=torch.float16)
...
#         x0 = load_512(input_image, device=device).to(dtype=torch.float16)
...
for i in range(1000):
    try:
        input_image = f'../../data/id/{i:06d}.png'
        
        randn = random.randint(0, len(lines) - 1)
        target_prompt = lines[randn]
        print(f"[{i+1}/100] Applying '{target_prompt}' to {i:06d}.png...")
        
        x0 = load_512(input_image, device=device).to(dtype=torch.float16).to(dtype=torch.float16)
        zs, wts = invert(x0=x0, prompt_src=source_prompt,
                         num_inference_steps=num_diffusion_steps,
                         cfg_scale_src=source_guidance_scale)
        if reconstruct:
            ddpm_out_img = sample(zs, wts, prompt_tar=target_prompt,
                                 skip=skip_steps, cfg_scale_tar=target_guidance_scale)
            
            ddpm_out_img.save(f'../../data/makeup_image/{i:06d}.png')
            
    except Exception as e:
        print(f"Skipping {i:06d}.png due to error: {e}")
        continue
